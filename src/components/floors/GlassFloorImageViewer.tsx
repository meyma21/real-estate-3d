          ...
          {floorImages.map((imageData, index) => (
            <SwiperSlide key={imageData.id || index} className="relative">
              ...
              >
                ...
                {/* Glass Hotspots */}
                {showHotspots && imageData.hotspots && Array.isArray(imageData.hotspots) && imageData.hotspots.length > 0 && (
                  <div className="absolute inset-0 pointer-events-none z-20">
                    {imageData.hotspots.map((hotspot, hotspotIndex) => {
                      const apartment = apartments.find(apt => apt.id === hotspot.apartmentId);
                      console.log('🔍 Rendering glass hotspot:', hotspot, 'apartment:', apartment);
                      return (
                        <motion.div
                          key={`hotspot-${hotspotIndex}-${hotspot.id || hotspotIndex}`}
                          initial={{ opacity: 0, scale: 0 }}
                          animate={{ opacity: 1, scale: 1 }}
                          transition={{ delay: 0.5 + hotspotIndex * 0.1, duration: 0.5 }}
                          ...
                          onClick={(e) => {
                            e.preventDefault();
                            e.stopPropagation();
                            console.log('🔍 Hotspot clicked directly!', hotspot);
                            alert('Hotspot clicked!'); // Test alert
